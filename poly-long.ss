...
(define (simplify p combiner eq-test? selector)
  (define (simplify-helper p result)
    (if (null? p) result
        (let ((simplified
               (combiner 
                (collect-like (pcar p) p eq-test? selector)
                )
               )
              )
...
(define (combine-terms like-terms)
  (let ((combined-coef 
         (foldr + 0 (map coef like-terms)
                )
         )
        )
    (if (zero? combined-coef) '()
        (mk-term combined-coef 
                 (vars (pcar like-terms))
                 )
        )
    ))
...
(define (negate-poly p) 
  (map 
   (lambda (x) (mk-term (* -1 (coef x)) (vars x))) 
   p)
  )
