...
(define (simp-terms p) (simplify p combine-terms vars= (lambda (term) (vars term))))
(define (simplify p combiner eq-test? selector)
  (define (simplify-helper p result) (if (null? p) result
        (let ((simplified (combiner (collect-like (pcar p) p eq-test? selector))))
          (if (null? simplified) result
              (simplify-helper (remove-like simplified (pcdr p) eq-test? selector) (cons simplified result))))))
  (simplify-helper p '()))
(define (combine-terms like-terms) (let ((combined-coef (foldr + 0 (map coef like-terms))))
    (if (zero? combined-coef) '() (mk-term combined-coef (vars (pcar like-terms))))))
...
(define (simp-vars p)
  (define (simp-vars-helper p result)
    (cond ((null? p) result) ((constant? (pcar p)) (cons (pcar p) (simp-vars (pcdr p))))
          (else (simp-vars-helper
                 (pcdr p)
                 (cons (mk-term (coef (pcar p)) (simplify (vars (pcar p)) combine-vars equal? (lambda (v) (vcar v))))
		       result)))))
  (simp-vars-helper p '()))
(define (combine-vars like-vars) (let ((combined-pow (foldr + 0 (map power like-vars))))
    (if (zero? combined-pow) '() (mk-var (letter (vcar like-vars)) combined-pow))))
(define (term* t1 t2) (mk-term (* (coef t1) (coef t2)) 
...
(define (v= v1 v2) (and (equal? (letter v1) (letter v2)) (= (power v1) (power v2))))
...
(define (constant? term) (cond ((null? term) #f) ((null? (coef term)) #f)
			       (else (and (integer? (coef term))  (no-vars? (vars term))))))
